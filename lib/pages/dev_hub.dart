...
import 'package:nexplay/models/price_model.dart';
import 'package:nexplay/models/search_mode.dart' as sm;
import 'package:nexplay/pages/game_detail.dart';
import 'package:skeletonizer/skeletonizer.dart';
...
class _DevelopersPageState extends State<DevelopersPage> {
  ...
  // Add searchGames function
  Future<sm.SearchModel?> _fetchGameDetails(String gameName) async {
    try {
      final searchResults = await _apiService.searchGames(gameName);
                                  ...
                                  itemBuilder: (context, gameIndex) {
                                    ...
                                    return Container(
                                      ...
                                      child: Skeletonizer(
                                        ...
                                        child: InkWell(
                                          onTap: () async {
                                            sm.SearchModel? gameDetails = await _fetchGameDetails(game.name);
                                            if (gameDetails != null) {
                                              Get.to(() => GameDetail(
                                                    game: GameModel(
                                                      id: gameDetails.id,
                                                      ...
                                                      ratingsCount: gameDetails.ratingsCount ?? 0,
                                                      rating: gameDetails.rating ?? 0,
                                                      esrbRating: EsrbRating(id: 2, name: 'Everyone', slug: 'everyone'),
                                                      shortScreenshots: gameDetails.shortScreenshots.map((screenshot) => ShortScreenshot(id: screenshot.id, image: screenshot.image!)).toList(),
                                                    ),
                                                    price: price,
                                                  ));
                                            }
                                          ...
                                          child: Column(
                                            children: [
                                              gameDescription == null
                                                  ? Container(height: 230, width: 195, color: Colors.grey.shade300)
                                                  : ClipRRect(
                                                      borderRadius: BorderRadius.circular(15),
                                                      child: Image.network(
                                                        gameDescription.backgroundImage!,
                                                        height: 230,
                                                        width: 190,
                                                        fit: BoxFit.cover,
                                                      ),
